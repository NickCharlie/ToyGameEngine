        ...
        namespace Collision
        {
            ...
            bool gjk(const L &points0, const R &point1)
            {
                ...
                while (true)
                {
                    ...
                    distance[0] = Geometry::distance(end, triangle[0], triangle[1], true);
                    distance[1] = Geometry::distance(end, triangle[1], triangle[2], true);
                    distance[2] = Geometry::distance(end, triangle[0], triangle[2], true);
            ...
            double epa(const L &points0, const R &points1, Geometry::Vector &vec)
            {
                ...
                while (true)
                {
                    distance[0] = Geometry::distance(end, points.front(), points.back(), true);
                    index = points.size();
                    for (size_t i = 1, count = points.size(); i < count; ++i)
                    {
                        distance[1] = Geometry::distance(end, points[i - 1], points[i], true);
